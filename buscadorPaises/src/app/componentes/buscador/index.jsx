...
class Buscador extends React.Component
{
	...
	{
		if (nombre!='')
			return this.paises.filter((pais)=>pais.like('%'+nombre+'%'));
		else
			return [];
	}
	...
	changeText(e)
	{
		...
		let paises = this.filtrarPais(texto);
	...
	render()
	{
		...
		if (this.state.paises.length==0)
			paises = <span><b>No se encontraron paises.</b></span>;
		else
			paises = this.state.paises.map((pais,index)=><li key={index}>{pais}</li>);
